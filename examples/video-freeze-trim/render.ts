/**
 * Davidup — trimmed clip with freeze frame sample (v0.2 S10).
 *
 *   bun run examples/video-freeze-trim/render.ts
 *
 * Renders composition.ts (a 1s trimmed window inside a 3s composition, which
 * freezes on the last extracted frame for the remaining 2s) to
 * output/video-freeze-trim.mp4 via skia-canvas + ffmpeg. See composition.ts
...
import { buildVideoFreezeTrimComposition } from "./composition.js";

const HERE = dirname(fileURLToPath(import.meta.url));
const OUTPUT_DIR = resolve(HERE, "output");
const VIDEO_MP4 = join(OUTPUT_DIR, "video-freeze-trim.mp4");

async function main(): Promise<void> {
  const comp = buildVideoFreezeTrimComposition();
