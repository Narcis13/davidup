/**
 * Davidup v0.3 — 30-second, two-template demo, MP4 flow.
 *
 *   bun run examples/two-templates-30s.ts
 *
 * Reads `two-templates-30s.json` (canonical authored source — exactly two
 * `$template` instances), validates it after the precompile pass, prints the
 * source line count to prove the <100-line claim, and renders the full clip
 * to `output/two-templates-30s.mp4` via skia-canvas + ffmpeg.
 *
 * `renderToFile` calls `precompile()` internally (drivers/node/index.ts §10),
 * so the file you author is the file you ship.
 */
...
const SOURCE = resolve(HERE, "two-templates-30s.json");
const OUTPUT_DIR = resolve(HERE, "output");
const PREVIEW_PNG = join(OUTPUT_DIR, "two-templates-30s.frame-1s.png");
const VIDEO_MP4 = join(OUTPUT_DIR, "two-templates-30s.mp4");

// Font assets in the JSON are referenced via portable relative paths
// (`./fonts/…`). skia-canvas resolves them against process.cwd, which is
// fragile, so we rewrite each asset to an absolute path keyed by id.
...
const SOURCE_LINE_LIMIT = 100;

async function main(): Promise<void> {
  const raw = await readFile(SOURCE, "utf8");
  const lineCount = raw.split("\n").length;
  const claim = lineCount < SOURCE_LINE_LIMIT ? "✓" : "✗";
  console.log(
    `[davidup] source ${SOURCE.replace(`${HERE}/`, "")} = ${lineCount} lines ` +
      `${claim} <${SOURCE_LINE_LIMIT}`,
  );
  if (lineCount >= SOURCE_LINE_LIMIT) {
    console.error(`[davidup] source exceeds ${SOURCE_LINE_LIMIT} lines — failing the test claim.`);
    process.exit(1);
  }
  ...
  console.log(
    `[davidup] precompile + validate ok: ${Object.keys(compiled.items).length} items, ` +
      `${compiled.tweens.length} tweens (all from 2 templates), ` +
      `${compiled.composition.duration}s @ ${compiled.composition.fps}fps`,
  );

  await mkdir(OUTPUT_DIR, { recursive: true });

  console.log(`[davidup] preview frame at t=1.0s → ${PREVIEW_PNG}`);
  const preview = await renderPreviewFrame(compiled, 1.0, { format: "png" });
