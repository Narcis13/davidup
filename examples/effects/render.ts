/**
 * Davidup — per-item effects sample (v1.1 S21).
 *
 *   bun run examples/effects/render.ts
 *
 * Renders composition.ts (blur focus pull, drop-shadowed card, glowing title)
 * to output/effects.mp4 via skia-canvas + ffmpeg. See composition.ts for the composition itself.
 */
...
import { buildEffectsComposition } from "./composition.js";

const HERE = dirname(fileURLToPath(import.meta.url));
const OUTPUT_DIR = resolve(HERE, "output");
const VIDEO_MP4 = join(OUTPUT_DIR, "effects.mp4");

async function main(): Promise<void> {
  const comp = buildEffectsComposition();
