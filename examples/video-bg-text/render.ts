/**
 * Davidup — video background + text overlay sample (v0.2 S10).
 *
 *   bun run examples/video-bg-text/render.ts
 *
 * Renders composition.ts (a full-frame video with a tweened text caption
 * fading + sliding in on top) to output/video-bg-text.mp4 via skia-canvas +
 * ffmpeg. See composition.ts for the composition itself.
 */
...
import { buildVideoBgTextComposition } from "./composition.js";

const HERE = dirname(fileURLToPath(import.meta.url));
const OUTPUT_DIR = resolve(HERE, "output");
const VIDEO_MP4 = join(OUTPUT_DIR, "video-bg-text.mp4");

async function main(): Promise<void> {
  const comp = buildVideoBgTextComposition();
