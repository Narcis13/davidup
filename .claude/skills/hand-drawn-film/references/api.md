...
- `trimGhosts(heard, { dur, voiced } = {})` trimGhosts(heard, { dur, voiced }) => the words minus the ones the audio never spoke: the trailing run that is zero-length or starts within 0.05 s of the file's end (whisper, given a prompt, repeats it there), and any word starting ...
- `checkFitted(A, voiced)` checkFitted(A, voiced): throws when the fitted words (first t0 to last t1) cover under half the voiced span, which is a tool's timing gone wrong (ghost words piled at one instant), not a take.
...
- `dialogueShot(opts)` **AY** AY. Dialogue shot (a line a beat): two actors (`actor` on the left, `other` on the right; by default two stick puppets, sam and a child, kit; `other: false` for one alone, standing at `x` or in the middle, every line its own) stand `h` ... Options: dur, lines, actor, other, audience, x, ground, h, gaze, gap, at, floor, seed.
