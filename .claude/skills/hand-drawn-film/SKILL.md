...
| `moon.js` | whiteboard, chalkboard | **the 4.0 film**: a 108 s lesson for kids-7 in four chapters written from its brief (`moon.md`, `hdf script`); a rig-sheet puppet that talks (`moon/` has the sheets and the script that draws them); recipes built on with `titlePlan`, `labelledPlan`, `countingPlan` (a ring round a label, the pen's sounds, a pop a shape); the golden samples 24 frames |
| `sam-moon.js` | chalkboard, alpha | the moon film's sam as an overlay clip cut to a composition's markers (`atMark`); `examples/hdf-moon/build.mjs` puts it over a photograph of the moon through davidup's MCP server |
