...
Group opacity is multiplicative alpha by default (R-20): a group's `transform.opacity` multiplies into each descendant's own alpha rather than flattening the group to one layer and applying opacity once. Overlapping children inside the same group therefore blend against each other at full strength before the group's opacity is applied on top, so a 50%-opacity group with two overlapping children is *not* visually equivalent to compositing the whole group as a single flattened layer at 50%.

Since v1.1 the flattened reading is available as an opt-in: `isolate: true` on a group paints its children onto a canvas-sized scratch surface (seeded with the matrix they would have inherited) and composites that surface once, carrying the group's opacity and its optional `blendMode`. The overlap seam disappears and a child's own blend mode sees only its siblings, not the canvas backdrop — CSS `isolation: isolate`. The default is unchanged and an opaque isolated group is pixel-identical to the multiplicative path, so nothing that already renders correctly moves. Isolation needs the optional `getTransform`/`setTransform` pair on `Canvas2DContext` plus an offscreen factory; a host supplying neither keeps the multiplicative path.

Effects (v1.1): any item may carry `effects`, an ordered stack of `{ type: "blur", radius }`, `{ type: "shadow", color, blur?, offsetX?, offsetY? }` and `{ type: "glow", color, radius }`. The renderer reuses the isolation machinery: the item (a group with all its children) is flattened onto a scratch surface at full alpha, each effect is applied in order, and the result is composited once with the item's opacity, so the item and its shadow fade as one and a group with effects is implicitly isolated. Blur runs in-engine (`src/engine/blur.ts`, the Filter Effects spec's three-box approximation of a Gaussian, on premultiplied pixels) because skia-canvas's `ctx.filter` blur on `drawImage` runs at half Chromium's σ. Shadow and glow use the Canvas2D shadow state, which both hosts rasterize alike. Effect parameters tween as `effects.<index>.<field>`. Blur needs the optional `getImageData`/`putImageData` on `Canvas2DContext`; a host without them draws the item unblurred.
