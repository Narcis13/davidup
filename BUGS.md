...
| R-29 | P2 (live-drive) | Cross-session state leak — standalone server persists across agent conversations | **FIXED** v1.1 Session 22 — `reset` (no args) now clears compositions **and** the user template/scene/behavior registries (`scope: "compositions"` keeps them); standalone `--session-ttl <s>` / `DAVIDUP_SESSION_TTL` auto-resets everything after idle time, reported as `list_engine_capabilities.server.sessionIdleSeconds`. stdio is one connection per process, so the leak was "process outlives conversation"; the TTL is the lifecycle hook. | standalone server lifecycle |
