...
fitWords), so the transcriber's spelling never reaches the film. This file only transcribes.
    ...
    python3 cli/align.py work/line1.wav --model base.en --lang en
    python3 cli/align.py work/line2.wav --prompt "Ştefan, Ioana"     a spelling hint for names

The copy is not a prompt by default: given one, faster-whisper repeats it at the end of the file as
zero-length words.
...
def faster(path, prompt, model, lang):
    from faster_whisper import WhisperModel
    m = WhisperModel(model, device="cpu", compute_type="int8")
    segs, _ = m.transcribe(path, word_timestamps=True, language=lang, initial_prompt=prompt or None, beam_size=5)
    return [{"text": w.word.strip(), "t0": round(w.start, 3), "t1": round(w.end, 3)} for s in segs for w in (s.words or [])]


def timestamped(path, prompt, model, lang):
    import whisper_timestamped as wt
    audio = wt.load_audio(path)
    got = wt.transcribe(wt.load_model(model, device="cpu"), audio, language=lang, initial_prompt=prompt or None)
...
def main():
    ...
    ap.add_argument("wav")
    ap.add_argument("--prompt", default="", help="whisper's initial prompt, a spelling hint (off by default)")
    ...
    for name, fn in (("faster_whisper", faster), ("whisper_timestamped", timestamped)):
        try:
            ...
            continue
        json.dump(fn(a.wav, a.prompt, a.model, a.lang), sys.stdout)
        sys.stdout.write("\n")
        return 0
