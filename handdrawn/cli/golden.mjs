...
// film under another look is a golden of its own, goldens/<film>-<look>.json (mini-paperInk~hand:test.json).
...
export const goldenPath = (path, film, { look } = {}) => join(dirname(resolve(path)), 'goldens', `${variant(film, { look })}.json`);

// { frames: { "0": sha256, ... }, wav: sha256 | null }
export async function goldenOf(path, film, { workers = defaultWorkers(), cacheMb = 512, look } = {}) {
  const frames = {};
  let prev = null;
  await produceFrames(path, film, { width: GOLDEN_WIDTH, workers, cacheMb, look }, (i, buf, dup) => {
...
export async function run([path, mode], flags, { loadFilm }) {
  ...
  const file = goldenPath(path, film, { look: flags.look });
  const got = await goldenOf(path, film, { workers: flags.workers, cacheMb: flags.cacheMb, look: flags.look });
  if (mode === 'write') {
  ...
  if (!existsSync(file)) throw new Error(`golden: no ${file}; run 'hdf golden ${path} write${flags.look ? ` --look '${flags.look}'` : ''}' first`);
