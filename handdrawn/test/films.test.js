...
test('films/ holds the seven ported films, fox-and-teapot and cutout-fox, each with a golden', async () => {
  ...
  assert.deepEqual(all.map((f) => f.name).sort(), ['asking', 'chapters', 'cutout-fox', 'fly-style', 'follow', 'four-looks', 'fox-and-teapot', 'fox-wave', 'gallop', 'growing', 'held-once', 'hello', 'holding', 'lesson', 'marked', 'mini', 'mini-voice', 'moon-book', 'narrated', 'on-beat', 'one-year', 'pointing', 'quiz-time', 'sums', 'walk-on', 'written']);
...
// 4.0 L3: mini in crayon is a golden of its own (goldens/mini-crayon.json, written on arm64).
test("mini under --look crayon matches its own golden, and lints clean", { skip: process.platform !== 'darwin' && 'goldens are written on darwin-arm64' }, async () => {
  assert.ok(existsSync(join(FILMS, 'goldens', 'mini-crayon.json')));
  const r = hdf('golden', 'films/mini.js', 'check', '--workers', '4', '--look', 'crayon');
  assert.equal(r.status, 0, r.stdout + r.stderr);
  const l = hdf('lint', 'films/mini.js', '--look', 'crayon');
  assert.equal(l.status, 0, l.stdout + l.stderr);
});
