...
test('films/ holds the seven ported films, fox-and-teapot and cutout-fox, each with a golden', async () => {
  ...
  assert.deepEqual(all.map((f) => f.name).sort(), ['chapters', 'cutout-fox', 'fly-style', 'four-looks', 'fox-and-teapot', 'fox-wave', 'gallop', 'growing', 'held-once', 'lesson', 'mini', 'mini-voice', 'moon-book', 'narrated', 'one-year', 'pointing', 'walk-on', 'written']);
