...
test('films/ holds the seven ported films, fox-and-teapot and cutout-fox, each with a golden', async () => {
  ...
  assert.deepEqual(all.map((f) => f.name).sort(), ['asking', 'chapters', 'cutout-fox', 'fly-style', 'four-looks', 'fox-and-teapot', 'fox-wave', 'gallop', 'growing', 'held-once', 'hello', 'lesson', 'marked', 'mini', 'mini-voice', 'moon-book', 'narrated', 'one-year', 'pointing', 'quiz-time', 'sums', 'walk-on', 'written']);
