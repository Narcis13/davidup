...
must_haves:
  truths:
    ...
    - "Remote assets are cached during render via existing AssetManager"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/api/services/job-queue.ts"
      to: "src/api/services/webhook.ts"
      via: "jobQueue.on('job:completed') calls deliverWebhook"
      pattern: "deliverWebhook"
    - from: "src/api/services/job-queue.ts"
      to: "src/render/asset-manager.ts"
      via: "renderVideo uses AssetManager internally for caching"
      pattern: "AssetManager"
      note: "ASST-04: AssetManager already caches remote assets during render - no new code needed"
---

<objective>
Implement render routes (POST /render, GET /render/:jobId) with webhook delivery.

Purpose: Core API endpoints for submitting render jobs and checking status. Covers API-02 through API-06 and ASST-04 (asset caching via existing AssetManager).
...
<task type="auto">
  <name>Task 2: Create render routes with webhook wiring</name>
  <files>
    - src/api/routes/render.ts
    - tests/api/render-routes.test.ts
  </files>
...
**Note on ASST-04 (remote asset caching):** The existing `AssetManager` in `src/render/asset-manager.ts` already implements caching for remote assets. When `loadImage(src)` is called, it checks `this.images.has(src)` and returns cached images. The render pipeline uses this AssetManager internally, so ASST-04 is satisfied by existing code. No new implementation needed - just verify the wiring exists.
  </action>
  <verify>
- `npm test tests/api/render-routes.test.ts` passes
- `npx tsc --noEmit` passes
- Verify webhook tests cover: completion delivery, failure delivery, no delivery when not configured
  </verify>
  <done>
POST /render and GET /render/:jobId implemented with validation, job queuing, status tracking, and webhook wiring verified by tests.
  </done>
</task>
...
3. Webhook wiring tests pass: verify webhook delivery on job:completed and job:failed events
4. TypeScript compiles: `npx tsc --noEmit`
5. End-to-end: POST /render returns job_id, GET /render/:jobId returns status
6. ASST-04 verified: AssetManager caches images via `this.images.set(src, image)` pattern
</verification>
...
- Webhook delivered on job completion with retries (explicitly tested)
- Webhook delivered on job failure with error details (explicitly tested)
- All routes protected by auth and rate limiting
- Remote assets cached during render via existing AssetManager (ASST-04)
</success_criteria>
