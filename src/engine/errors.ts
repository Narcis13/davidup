...
//
// Lives in `engine/` so the compose layer (which sits *below* mcp) can throw
// structured errors without an upward `compose → mcp` import. `src/mcp/errors.ts`
// re-exports the symbols so existing MCP-layer imports keep working unchanged.
